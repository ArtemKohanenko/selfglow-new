...
export async function getAllTarifsForManuallySubscriber(ctx) {
	// Fetch all tariffs
	...
	pairs.forEach((pair, i) => {
		...
		row.push({
			text: buttonText,
			callback_data: `selectTarifManually ${pair[1]}`,
		})
		keyboard.push(row)
	})

	// Send the message with the keyboard
	await ctx.reply('<b>Выберите тарифный план, который будет выдан пользователю:</b>', {
		reply_markup: {
