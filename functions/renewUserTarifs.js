...
export async function renewUserTarifs(ctx, userId) {
	...
	pairs.forEach((pair, i) => {
		...
		row.push({
			text: buttonText,
			callback_data: `renewTarif ${pair[1]}`,
		})
		keyboard.push(row)
	})
	keyboard.push([{
	...
	await ctx.reply('<b>Выберите тариф пользователя, который хотите продлить:</b>', {
		reply_markup: {
